...
function drlam_seo_check_edit($post,$content,$h1_title,$keyword,$categories,$page_title,$slug,$meta_description)
{
...
/*		$x = new DOMDocument("1.0", "utf-8");
		...
		$data = $wpdb->get_results( "SELECT * FROM `wp_postmeta` WHERE meta_key='_yoast_wpseo_title' and post_id='".$id."' " );
		$meta_title=strtolower($data[0]->meta_value);
...
//	echo print_r($data);
	...
	{
		$each_mk=strtolower($value);
		$pos = strpos(str_replace("-"," ",$slug), $each_mk);
		if($pos!==false)
		{
			$stat = true;
			$found = $each_mk;
